...
// struct pathelement *point;
// point = pathlist;
// while(point != NULL){
//   printf("%s\n", point->element);
//   point = point->next;
// }
// memcpy(prompt, "\0", strlen(prompt));
// strncpy(prompt, owd, strlen(owd));
// memcpy(prompt+strlen(prompt)-1)

//deletes the entire pathlist, freeing up space
void delete_path(struct pathelement *pathlist){
  struct pathelement *tmp = pathlist, *tmpPrev;

  tmpPrev = tmp;
  free(tmpPrev->element);
  while(tmp != NULL){
    tmpPrev = tmp;
    tmp = tmp->next;
    free(tmpPrev);
